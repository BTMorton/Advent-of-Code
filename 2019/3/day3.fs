...
let getWirePosition (currentX, currentY) (movementX, movementY) =
    (currentX + movementX), (currentY + movementY)

let reduceWirePositions (wires: Wires) (movement: WirePoint) =
    let currentPosition = wires.currentPosition
    let newPosition = getWirePosition currentPosition movement

    match movement with
    | (x, 0) ->
        let hLines = List.append wires.hLines [{
            startPoint= currentPosition
            endPoint = newPosition
            steps = wires.currentSteps
        }]
        {
            currentPosition = newPosition
            currentSteps = wires.currentSteps + abs(x)
            hLines = hLines
            vLines = wires.vLines
        }
    | (0, x) ->
        let vLines = List.append wires.vLines [{
            startPoint = currentPosition
            endPoint = newPosition
            steps = wires.currentSteps
        }]
...
let getWireLines (wire: string list) =
    let wireCommands = wire |> List.map (splitWireCommand >> getCommandMovement)
    wireCommands
        |> List.fold reduceWirePositions {
            currentPosition = (0,0)
            currentSteps = 0
            hLines = []
            vLines = []
...
let isWithinX xValue line =
    let (startX, _) = line.startPoint
    let (endX, _) = line.endPoint
    (startX < xValue  && xValue < endX)
    || (endX < xValue && xValue < startX)

let isWithinY yValue line =
    let (_, startY) = line.startPoint
    let (_, endY) = line.endPoint
    (yValue > startY && yValue < endY)
    || (yValue > endY && yValue < startY)

let doLinesCross hLine vLine =
    isWithinY (snd hLine.startPoint) vLine
    && isWithinX (fst vLine.startPoint) hLine

let findCrossings (vLines: WireLine list) (hLine: WireLine) =
    vLines
        |> List.filter (doLinesCross hLine)
        |> List.map (fun vLine ->
            let vSteps = abs(fst vLine.startPoint - fst hLine.startPoint)
            let hSteps = abs(snd hLine.startPoint - snd vLine.startPoint)
            {
                point = (fst vLine.startPoint, snd hLine.startPoint)
                steps = vLine.steps + hLine.steps + vSteps + hSteps
            }
        )

let findWireCrossings (hLines: WireLine list) (vLines: WireLine list) =
    let points = hLines |> List.map (findCrossings vLines)
    points |> List.reduce (@)

let findCrossedPoints (source: Wires) (compare: Wires) =
    (findWireCrossings source.hLines compare.vLines) @ (findWireCrossings compare.hLines source.vLines)

let crossingDistance crossing =
    let (pointX, pointY) = crossing.point
    abs(pointX) + abs(pointY)

let stepsDistance (crossing: WireCrossing) = crossing.steps

let getClosestCrossingByDistance wireData1 wireData2 =
    let wire1 = getWireLines wireData1
    let wire2 = getWireLines wireData2

    let crossings = findCrossedPoints wire1 wire2
    crossings |> List.minBy crossingDistance

let getClosestCrossingBySteps wireData1 wireData2 =
    let wire1 = getWireLines wireData1
    let wire2 = getWireLines wireData2

    let crossings = findCrossedPoints wire1 wire2
    crossings |> List.minBy stepsDistance

[<EntryPoint>]
let main argv =
    // let testData1 = ["R8";"U5";"L5";"D3"]
    // let testData2 = ["U7";"R6";"D4";"L4"]
    // let testClosest = getClosestCrossing testData1 testData2
    // printfn "Test closest is %A distance %d" testClosest (crossingDistance testClosest)

    // let testData1 = ["R75";"D30";"R83";"U83";"L12";"D49";"R71";"U7";"L72"]
    // let testData2 = ["U62";"R66";"U55";"R34";"D71";"R55";"D58";"R83"]
    // let testClosest = getClosestCrossing testData1 testData2
    // printfn "Test closest is %A distance %d" testClosest (crossingDistance testClosest)

    // let testData1 = ["R98";"U47";"R26";"D63";"R33";"U87";"L62";"D20";"R33";"U53";"R51"]
    // let testData2 = ["U98";"R91";"D20";"R16";"D67";"R40";"U7";"R15";"U6";"R7"]
    // let testClosest = getClosestCrossing testData1 testData2
    // printfn "Test closest is %A distance %d" testClosest (crossingDistance testClosest)

    let closest = getClosestCrossingByDistance wireData1 wireData2
    printfn "Closest point is %A, distance %d" closest (crossingDistance closest)

    let closest = getClosestCrossingBySteps wireData1 wireData2
    printfn "Closest point is %A, steps %d" closest (stepsDistance closest)
